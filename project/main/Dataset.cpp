...
void Dataset::fillDataset(cv::Size_<unsigned> frame_size, ushort depth, ushort cn, std::string dataset_path,
    ...
    datasetStruct.m__directory_path_input = datasetStruct.m_dataset_path.string() + unterordner + std::string("/dummy.txt");
    //data/stereo_flow/image_02
    datasetStruct.m__directory_path_input = datasetStruct.m__directory_path_input.parent_path();

    //data/stereo_flow/
    datasetStruct.m__directory_path_gt_flow = datasetStruct.m__directory_path_input.parent_path();

    //data/stereo_flow/
    datasetStruct.m__directory_path_gt_trajectory = datasetStruct.m__directory_path_input.parent_path();
    ...
    if (boost::filesystem::exists(datasetStruct.m__directory_path_input) == 0 ) {
    boost::filesystem::create_directories(datasetStruct.m__directory_path_input.string());
    }
...
const boost::filesystem::path Dataset::getInputPath() {
    return datasetStruct.m__directory_path_input;
}
