...
std::vector<float> KittiSubmitInterface::flowErrorsOutlier (FlowImageExtended &F_gt,FlowImageExtended &F_orig,
                                                        FlowImageExtended &F_ipol) {
...
std::vector<float> KittiSubmitInterface::flowErrorsAverage (FlowImageExtended &F_gt,FlowImageExtended &F_orig,FlowImageExtended &F_ipol) {
...
void KittiSubmitInterface::plotVectorField (FlowImageExtended &F,string dir,char* prefix) {
...
bool KittiSubmitInterface::eval (string result_sha, Mail* mail) {
    ...
    std::vector< std::vector<float> > errors_noc_out;
    std::vector< std::vector<float> > errors_occ_out;
    std::vector< std::vector<float> > errors_noc_avg;
    std::vector< std::vector<float> > errors_occ_avg;
    ...
    for (int32_t i=0; i<195; i++) {
        ...
        try {
            ...
            FlowImageExtended F_gt_noc(gt_noc_dir + "/" + prefix + ".png");
            FlowImageExtended F_gt_occ(gt_occ_dir + "/" + prefix + ".png");
            ...
            FlowImageExtended F_orig(image_file);

            // interpolate missing values
            FlowImageExtended F_ipol(F_orig);
            F_ipol.interpolateBackground();

            // add flow errors
            std::vector<float> errors_noc_out_curr = flowErrorsOutlier(F_gt_noc,F_orig,F_ipol);
            std::vector<float> errors_occ_out_curr = flowErrorsOutlier(F_gt_occ,F_orig,F_ipol);
            std::vector<float> errors_noc_avg_curr = flowErrorsAverage(F_gt_noc,F_orig,F_ipol);
            std::vector<float> errors_occ_avg_curr = flowErrorsAverage(F_gt_occ,F_orig,F_ipol);
