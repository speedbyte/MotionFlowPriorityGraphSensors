...
class CanDriver(object):
    ...
    """
    def __init__(self, baudrate = '500'):
        """
        ...
        self.writeOps = 0
        if ( baudrate == '500'):
            self.CanBaud = CAN_BAUD._500K
        elif ( baudrate == '125'):
            self.CanBaud = CAN_BAUD._125K
        elif ( baudrate == '100'):
            self.CanBaud = CAN_BAUD._100K
    ...
    def open(self, device):
        ...
        result = ctypes.windll.Pcan_usb.CAN_Init(self.CanBaud,CAN_INIT_TYPE_ST)
        ...
        self.isOpen = True
        if ( self.CanBaud == CAN_BAUD._500K ):
            print "Initialising Peak with Baud, 500K"
        elif ( self.CanBaud == CAN_BAUD._125K ):
            print "Initialising Peak with Baud, 125K"
        elif ( self.CanBaud == CAN_BAUD._100K ):
