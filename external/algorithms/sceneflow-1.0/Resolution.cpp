...
inline  unsigned char cvmGet8u( const CvMat* mat, int row, int col )
{
...
inline  static void  cvmSet8u( CvMat* mat, int row, int col, unsigned char value )
{
    assert( (unsigned)row < (unsigned)mat->rows &&
            (unsigned)col < (unsigned)mat->cols );
    assert( CV_MAT_TYPE(mat->type) == CV_8UC1 );
...
inline  static void  cvmSet8u_borders( CvMat* mat, unsigned char value )
{
    int rows  = mat->rows;
    int cols = mat->cols;
    int i, j;
